...
-- ---------------------------------------------------------------------------
-- 7. النصوص العربية تصل سليمة
--
-- أوّل نشر لهذه المهاجرة أخرج التسميات '?????' في الإنتاج: سكربت النشر كان
-- يبني جسم الطلب عبر stdout ثم التقاط الصدفة، وويندوز يحوّل غير اللاتيني
-- إلى '?' في تلك الرحلة. لم يظهر الأثر من قبل لأن العربية كانت في التعليقات
-- وحدها؛ وهذه أوّل مرّة تصل فيها إلى بيانات يراها المستخدم.
--
-- الفحص بالبايتات لا بالمظهر: النصّ العربي متعدّد البايتات بالضرورة، فتساوي
-- عدد المحارف والبايتات يعني أنه فُقد.
-- ---------------------------------------------------------------------------

select pg_temp.ok(
  (select octet_length(owner_hint) > length(owner_hint)
   from public.commission_finalization_blockers('0a000000-0000-0000-0000-0000000000e3')
   limit 1),
  'تسمية الجهة نصٌّ عربي سليم لا محارف مفقودة');

select pg_temp.ok(
  (select octet_length(action_hint) > length(action_hint)
   from public.commission_finalization_blockers('0a000000-0000-0000-0000-0000000000e3')
   limit 1),
  'نصّ الإجراء عربي سليم');
