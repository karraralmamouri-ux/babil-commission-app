...
| INS-013 | Recurring monthly path consumes the raw SaaS Activation file — no hand-made Remaining column | PASS | `ensure_installation_state_for_enrollment`, `20261103090000_installation_raw_activation_bridge.sql` | `trg_installation_enrollment_opens_state` after insert on `installation_enrollments` | — | `tests/sql/installation-raw-activation-bridge.sql` §1, §3, §4 | Opening Remaining comes from `installation_stage_definitions.expected_remaining` of the published version — configuration, not a client value or a constant |
| INS-014 | Server derives current/next stage, Remaining and entitlement from authoritative state | PASS | `materialize_installation_entitlements`, `20260913090000_materialize_and_batch.sql` | reads `installation_subscriber_state`; no Remaining parameter exists on any recurring RPC | — | `tests/sql/installation-raw-activation-bridge.sql` §2, §4 | Whether each stage also needs a fresh qualifying event in its own period is DEC-007 — open; no gate added, `materialize_installation_entitlements` behaviourally unchanged |
| INS-015 | Progression P1→P2→P3→P4→DONE is deterministic, sequential, idempotent, auditable | PASS | `trg_payment_advances_installation_state`, `20261103090000_installation_raw_activation_bridge.sql` | after update on `installation_entitlements` paid transition; `installation_history_identity_key unique (subscriber_uuid, stage)`; `for update` on the state row | — | `tests/sql/installation-raw-activation-bridge.sql` §5–§9, `tests/sql/payout-end-to-end.sql` §11 | Before this, nothing advanced state at all, so `STAGE_OUT_OF_SEQUENCE` blocked every second stage permanently. Double advancement is prevented by the history unique key, not by trigger logic |
| INS-016 | Deriving an entitlement does not bypass invoice/hold/identity/ownership controls | PASS | `materialize_installation_entitlements`, `installation_entitlement_eligibility`, `record_installation_payment` | VERIFIED invoice per stage, effective holds, identity CONFLICT, `subscriber_ownership_type = RESELLER`, `guard_installation_payment` before update | — | `tests/sql/installation-raw-activation-bridge.sql` §10 | The bridge writes only subscriber/state rows; it adds no payment authority and touches no entitlement or ledger row |
